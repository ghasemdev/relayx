---
name: speckit-code-modernization-map
...
metadata:
  author: Anthropic / Spec Kit Community
  source: code-modernization:commands/modernize-map.md
---
...
```bash
TPL="${CLAUDE_PLUGIN_ROOT:-.specify/extensions/code-modernization}/assets/topology-viewer.html"
[ ! -f "$TPL" ] && TPL=$(find . -name "topology-viewer.html" 2>/dev/null | head -n 1)
python3 - "$TPL" analysis/$1 <<'EOF'
import json, sys, os
...
check that either `${CLAUDE_PLUGIN_ROOT}` or `.specify/extensions/code-modernization/`
exists — report that rather than hand-writing a viewer.
